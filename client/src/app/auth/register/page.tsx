...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Failed to register. Please check your details.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-950 p-4 text-slate-200">
      <div className="absolute top-0 left-1/2 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-purple-500/20 blur-[120px]"></div>
      ...
      <div className="z-10 w-full max-w-md overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/50 backdrop-blur-xl shadow-2xl">
        <div className="p-8">
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-slate-400">Join TeamVault and start organizing</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                required
                className="w-full rounded-lg border border-slate-700 bg-slate-800/50 px-4 py-2.5 text-white placeholder-slate-500 focus:border-purple-500 focus:outline-none focus:ring-1 focus:ring-purple-500 transition-colors"
